...
public class LobbyManager {
		...
		if (l.getBlock().getState() instanceof Sign){
			...
			if (cs != null){
				...
				if (type == LobbyType.STATUS){
					ls = new LobbySign(l.getBlockX(), l.getBlockY(), l.getBlockZ(), plugin, l.getWorld().getName(), cs.getString("displayname"), 0, type);
				}
				else if (type == LobbyType.PLAYERS){
					if (index >= 1){
						ls = new LobbySign(l.getBlockX(), l.getBlockY(), l.getBlockZ(), plugin, l.getWorld().getName(), cs.getString("displayname"), index, type);
					}
					else {
	...
	public void remove(LobbySign s){
		try {
			...
			y.load(f);
			Location3D l = new Location3D(y.getString(s.getIndex() + ".world"), y.getInt(s.getIndex() + ".x"), y.getInt(s.getIndex() + ".y"), y.getInt(s.getIndex() + ".z"));
			...
			for (String k : y.getKeys(false)){
				...
				if (w != null){
					if (y.get(k + ".arena") != null &&
						...
						LobbySign ls = new LobbySign(l.getBlockX(), l.getBlockY(), l.getBlockZ(), plugin, w.getName(), y.getString(k + ".arena"), y.getInt(k + ".number"), LobbyType.fromString(y.getString(k + ".type").toUpperCase()));
