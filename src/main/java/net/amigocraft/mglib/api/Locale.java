...
public class Locale {
	...
	public void initialize(){
		if (Locale.class.getClassLoader().getResource("locales") != null){
			...
			String defaultLocale = Minigame.getMinigameInstance(plugin) != null ? Minigame.getMinigameInstance(plugin).getConfigManager().getDefaultLocale() : "enUS";
			try {
				...
				is = Bukkit.getPluginManager().getPlugin(plugin).getClass().getResourceAsStream("/locales/" +
						Main.plugin.getConfig().getString("locale") + ".csv");
				if (is == null){
					MGUtil.log("Locale defined in config not found in JAR or plugin folder; defaulting to " + defaultLocale, prefix, LogLevel.WARNING);
					is = defaultIs;
				}
			}
			try {
				...
				if (defaultIs != null){
					...
					MGUtil.log("Neither the defined nor default locale could be loaded. " + "Localized messages will be displayed only as their keys!", prefix, LogLevel.SEVERE);
				}
			}
			catch (IOException e){
				e.printStackTrace();
